...
try {
  ...
  // Extreme compact density must be real geometry, not just a saved value.
  await page.locator('#ux-row-height').fill('20');
  ...
  // Fit can use the new 20px floor while remaining within valid bounds.
  ...
  // Display settings expose and apply the same floor.
  ...
  const appliedHeight = await page.locator(`[data-task-row="${id}"]`).evaluate((el) => el.getBoundingClientRect().height);
  assert.ok(appliedHeight >= 19 && appliedHeight <= 21, `display settings did not apply 20px: ${appliedHeight}`);
  ...
  // Mobile must remain document-overflow free.
