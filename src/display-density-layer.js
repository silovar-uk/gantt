(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .display-panel { position:absolute; z-index:80; top:calc(100% + 8px); right:0; width:min(390px, calc(100vw - 24px)); padding:14px; border:1px solid #dbe2ea; border-radius:13px; background:#fff; box-shadow:0 18px 46px rgba(15,23,42,.18); pointer-events:auto; }
      ...
      .density-preset { min-height:44px; padding:8px 7px; border:1px solid #d7e0eb; border-radius:9px; color:#475569; background:#fff; font-size:12px; font-weight:850; text-align:center; cursor:pointer; }
      ...
      body.gantt-density-compact .date-end,
      ...
      <div class="menu-wrap display-settings-wrap">
        ...
        <section id="display-settings-panel" class="display-panel" hidden aria-label="表示設定">
          <div class="display-panel__head">
            <div><strong>表示設定</strong><span>まずは軽量版です。行高・日付幅・左幅だけを安全に調整します。</span></div>
  ...
  function setPanelWidth(width) {
    ...
    const max = Math.max(340, Math.min(window.innerWidth * 0.72, 920));
    const value = clamp(width, 340, max);
  ...
  function reflowRows() {
    ...
    rows.forEach((row, index) => {
      ...
      if (bar) {
        ...
        setStyle(bar, 'height', `${Math.max(18, Math.min(26, rowHeight - 6))}px`);
      }
  ...
  function bindEvents() {
    ...
    $('#zoom-range')?.addEventListener('input', (event) => {
      settings.dayWidth = clamp(event.target.value, 10, 56);
      settings.density = settings.density === 'standard' ? 'standard' : 'custom';
      saveSettings();
      syncControls();
    });
