(() => {
  ...
  style.textContent = `
    ...
    [hidden], .modal[hidden], .share-modal[hidden], .project-library-modal[hidden], #json-modal[hidden], #share-modal[hidden] {
    ...
    .weekend-layer, .selected-row-band, .today-line, .today-dot, .holiday-band {
      pointer-events:none !important;
    }

    /* Header action safety: prevent export controls from being pushed off-screen. */
    .project-bar { flex-wrap:wrap; align-items:flex-start; gap:10px 14px; }
    .project-title-wrap { flex:1 1 320px; max-width:100%; }
    .project-title { width:100%; max-width:min(560px,100%); }
    .project-actions { flex:1 1 360px; min-width:0; display:flex; align-items:center; justify-content:flex-end; flex-wrap:wrap; gap:6px; }
    .project-actions .button, .project-actions .icon-button, .project-actions .menu-wrap { flex:0 0 auto; }
    ...
    #share-open-btn, #export-json-btn, #import-json-btn, #library-open-btn { min-width:84px; min-height:36px; padding-inline:11px; font-weight:850; }
    #share-open-btn { color:#fff; border-color:#244a8f; background:#244a8f; box-shadow:0 1px 1px rgba(36,74,143,.16); }
    #share-open-btn:hover { background:#1e3e78; border-color:#1e3e78; }
    #export-json-btn { border-color:#8fa1b7; background:#fff; }
    #export-json-btn::after { content:'▾'; margin-left:2px; color:#64748b; font-size:10px; }
    #library-open-btn { min-width:66px; color:#475569; background:#f8fafc; border-color:#dbe2ea; }
    #export-menu { left:auto; right:0; max-width:calc(100vw - 24px); }
    .project-actions .popup-menu { z-index:130; }
    .save-status { max-width:180px; overflow:hidden; text-overflow:ellipsis; }

    @media (max-width:1040px) { .save-status { flex-basis:100%; max-width:none; text-align:right; } }
    @media (max-width:860px) {
      .project-bar { padding:10px 12px; }
      .project-title-wrap { flex-basis:100%; }
      .project-actions { width:100%; flex-basis:100%; justify-content:flex-start; align-items:stretch; }
      .save-status { order:30; flex-basis:100%; font-size:11px; text-align:left; }
      #share-open-btn, #import-json-btn, #export-json-btn, #library-open-btn { min-width:0; width:100%; }
      .project-actions .action-share, .project-actions .action-import, .project-actions .action-export-wrap, .project-actions .action-library { flex:1 1 116px; }
      .project-actions .action-more-wrap { flex:0 0 38px; }
      #more-btn { width:38px; height:36px; }
    }
    @media (max-width:720px) { .library-open-button.action-library { display:inline-flex !important; } }
    @media (max-width:560px) {
      .project-actions { display:grid; grid-template-columns:1fr 1fr 40px; gap:6px; }
      .project-actions .action-share, .project-actions .action-export-wrap, .project-actions .action-import, .project-actions .action-library, .project-actions .action-more-wrap, #share-open-btn, #import-json-btn, #export-json-btn, #library-open-btn { width:100%; }
      ...
      .save-status { grid-column:1 / -1; grid-row:3; }
      .popup-menu, .popup-menu-right { position:fixed; left:10px !important; right:10px !important; top:auto !important; bottom:12px !important; width:auto !important; max-width:none; }
    }
  `;
  ...
  // Reorganize top header and command bar by role.
  loadScript('src/header-command-redesign-layer.js?v=20260709-header-command-v1');
  ...
  loadScript('src/share-safe-upgrade.js?v=20260709-share-step5-6-v1');
  // Step 7 restore: lightweight stepped JSON import guidance.
  loadScript('src/import-json-step-lite.js?v=20260709-import-step7-v1');
  ...
  window.GANTT_DESK_RESTORED_LAYERS = ['display-density-layer', 'display-settings-inline-layer', 'display-resize-control-layer', 'app-display-state-bridge', 'category-display-layer', 'display-menu-redesign-layer', 'header-command-redesign-layer', 'bulk-task-layer', 'calendar-highlight-layer', 'header-action-polish', 'share-safe-upgrade', 'import-json-step-lite'];
